...
const NEWS_SOURCES = [
  'reuters.com', 'ap.org', 'bbc.com', 'cnn.com', 'nytimes.com', 'wsj.com',
  'washingtonpost.com', 'ft.com', 'economist.com', 'time.com', 'newsweek.com'
];

const BUSINESS_NEWS = [
  'bloomberg.com', 'forbes.com', 'fortune.com', 'businessinsider.com',
  'techcrunch.com', 'venturebeat.com', 'wired.com', 'engadget.com'
];

// GEO (Generative Engine Optimization) indicators
const GEO_INDICATORS = [
  'featured snippet', 'knowledge graph', 'people also ask', 'structured data',
  'rich snippet', 'schema markup', 'answer box', 'overview', 'summary'
];
...
function isOwnedDomain(url: string, keyword: string): { isOwned: boolean; confidence: number; } {
  ...
  if (domain.includes('official') || url.includes('/about') || url.includes('/profile')) {
    confidence += 0.3;
    isOwned = true;
  }
  
  // Company-specific patterns
  if (url.includes('/team') || url.includes('/leadership') || url.includes('/founder')) {
    confidence += 0.2;
    isOwned = true;
  }
...
async function analyzeGEOPresence(results: SearchResult[], keyword: string): Promise<number> {
  let geoScore = 0;
  ...
  for (const result of results) {
    ...
    const geoMatches = GEO_INDICATORS.filter(indicator => text.includes(indicator)).length;
    ...
    if (text.includes('biography') || text.includes('profile of') || text.includes('about ' + keyword.toLowerCase())) {
      geoScore += 8;
    }
...
export async function analyzeEnhancedMetrics(
  ...
  const socialPresenceScore = Math.min(100, 
  ...
  const wikipediaScore = entityType === 'individual' ? 0 : 
  ...
  const ownedAssetsScore = Math.min(100, 
    (officialSites * 20) + 
    (verifiedProfiles * 15) + 
    (ownedDomains.length * 10) +
    (entityType === 'individual' ? 20 : 10) // Base score
  );
  ...
  for (const result of results) {
    ...
    if (NEWS_SOURCES.some(news => domain.includes(news)) || 
        BUSINESS_NEWS.some(biz => domain.includes(biz))) {
      newsResults++;
    }
    
    if (domain.includes('blog') || text.includes('blog post') || 
        result.source.toLowerCase().includes('blog')) {
      blogResults++;
    }
    
    if (text.includes('press release') || text.includes('announces') || 
        text.includes('pr newswire')) {
      prResults++;
    }
    
    if (text.includes('interview') || text.includes('talks with') || 
  ...
  return {
    ...
    geoPresence: {
      score: geoScore,
      aiOptimizedResults,
      structuredDataResults: results.filter(r => r.snippet.includes('•') || r.snippet.includes('|')).length,
      knowledgeGraphMentions: results.filter(r => 
