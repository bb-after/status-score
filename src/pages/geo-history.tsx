...
export default function GeoHistory() {
  ...
  const [selectedSearch, setSelectedSearch] = useState<GeoSearchRecord | null>(null);
        ...
        <Box>
          <HStack justify="space-between" align="center" mb={4}>
            <VStack align="start" spacing={1}>
              ...
              <Text color="gray.600">
                Review your previous GEO (Generative Engine Optimization) analyses
              </Text>
            </VStack>
            <Button
              leftIcon={<RepeatIcon />}
                  ...
                  <VStack align="start" spacing={2}>
                    ...
                    <HStack spacing={2} wrap="wrap">
                      {search.overallSentiment && (
                        <Badge
                          colorScheme={getSentimentColor(search.overallSentiment)}
                          variant="subtle"
                          fontSize="xs"
                        >
                          {search.overallSentiment}
                  ...
                  <VStack align="start" spacing={3}>
                    ...
                    {search.results && (
                      <Text fontSize="sm" color="gray.600">
                        {search.results.length} AI engine response{search.results.length !== 1 ? 's' : ''}
                      </Text>
                    )}
        ...
        <ModalContent maxH="90vh">
          <ModalHeader>
            <VStack align="start" spacing={2}>
              ...
              {selectedSearch && (
                <HStack spacing={3}>
                  <Badge colorScheme={getAnalysisTypeColor(selectedSearch.analysisType)}>
                ...
                <Card variant="outline">
                  ...
                  <CardBody>
                    <SimpleGrid columns={{ base: 1, md: 2 }} spacing={4}>
                      <Box>
                        <Text fontSize="sm" color="gray.600" mb={1}>Keyword</Text>
                        <Text fontWeight="medium">{selectedSearch.keyword}</Text>
                      </Box>
                      <Box>
                        <Text fontSize="sm" color="gray.600" mb={1}>Analysis Type</Text>
                        <Badge colorScheme={getAnalysisTypeColor(selectedSearch.analysisType)}>
                          {selectedSearch.analysisType}
                        </Badge>
                      </Box>
                      <Box>
                        <Text fontSize="sm" color="gray.600" mb={1}>Intent Category</Text>
                        <Text textTransform="capitalize">
                          {selectedSearch.intentCategory.replace(/_/g, ' ')}
                        </Text>
                      </Box>
                      <Box>
                        <Text fontSize="sm" color="gray.600" mb={1}>Overall Sentiment</Text>
                        {selectedSearch.overallSentiment ? (
                          <Badge colorScheme={getSentimentColor(selectedSearch.overallSentiment)}>
                            {selectedSearch.overallSentiment}
                          </Badge>
                        ) : (
                            ...
                            <VStack align="stretch" spacing={3}>
                              ...
                              {result.error ? (
                                ...
                                <Text fontSize="sm" color="gray.700" whiteSpace="pre-wrap">
                                  {result.summary || result.response || 'No response content'}
                                </Text>
                              )}
                              ...
                              {result.sources && result.sources.length > 0 && (
                                <Box>
                                  ...
                                  <VStack spacing={1} align="start">
                                    {result.sources.slice(0, 3).map((source: any, sourceIndex: number) => (
                    ...
                    <CardBody>
                      ...
                      >
                        <Text
                          ...
                          color="gray.600"
                        >
                          {JSON.stringify(selectedSearch.aggregatedInsights, null, 2)}
                        </Text>
                      </Box>
                    </CardBody>
                  </Card>
