...
export default function GeoHistory() {
  ...
  useEffect(() => {
    if (user?.isAuthenticated && !authLoading) {
      // Check if user has premium subscription before fetching
      if (hasActiveSubscription) {
        fetchHistory();
      } else if (!subscriptionLoading) {
        // Show paywall if no subscription and not loading
        onPaywallOpen();
      }
    }
    ...
    >
      <Container maxW="7xl" py={8}>
        ...
        <PaywallModal
          ...
          onClose={onPaywallClose}
          feature="GEO History"
          description="View and manage your previous GEO reputation analyses to track changes over time and understand regional reputation trends."
        />
      </Container>
    </ProtectedRoute>
  );
